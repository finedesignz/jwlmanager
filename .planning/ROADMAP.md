...
- [x] 04-01-PLAN.md — Core downgrade transform: remap closure (deterministic ORDER BY LocationId) + v14 DDL rebuild + SchemaDowngradeFailed + tests
- [x] 04-02-PLAN.md — dry_run_downgrade (reuse DryRunReport) + throwaway-copy save_v14_copy (session stays v16) + Tauri commands
- [x] 04-03-PLAN.md — Frontend "Save v14-compatible copy…" action + preview + Python differential oracle
...
- [x] 06-01-PLAN.md — Extract db/labels.rs + unify NotesRow into BrowseRow (enabling refactor, Notes stays green) (Wave 1)
- [x] 06-02-PLAN.md — Five verbatim category getters (browse.rs) + list_category command + identity-PK query tests (Wave 2)
- [x] 06-03-PLAN.md — Frontend: operations descriptor + CategorySwitcher + virtualized CategoryList + vitest (Wave 2)
- [x] 06-04-PLAN.md — Wire switcher + list_category into App shell, retire NotesList, DATA-07 end-to-end (Wave 3)
...
- [x] 07-03-PLAN.md — Tag add/remove/rename + archive-wide reorder + Utilities menu (EDIT-03, EDIT-04) (Wave 3)
- [x] 07-04-PLAN.md — Clean + Mask with typed-confirm gate (EDIT-06) (Wave 4)
- [x] 07-05-PLAN.md — Field-constrained record editor + remaining per-category deletes + cross-op round-trip suite (EDIT-07) (Wave 5)
...
- [x] 08-02-PLAN.md — Bookmarks (`¦` escaping, no end sentinel) + Annotations (bracket tags, `{END}`) export+import (Wave 2)
- [x] 08-03-PLAN.md — Highlights export+import; first production call site of the Phase 7 range union-merge primitive (Wave 3)
- [x] 08-04-PLAN.md — Notes export+import (multi-range `RANGE`, opt-in bucket delete) + five-category semantic round-trip (Wave 4)
- [x] 08-05-PLAN.md — `.jwlplaylist` container export+import with full re-keying, zip-slip-safe (Wave 5)
- [x] 08-06-PLAN.md — Playlist media add (hash dedup, staged-DB-then-files commit) + delete media ref-counting (Wave 6)
...
- [x] 11-01-PLAN.md
- [x] 11-02-PLAN.md
- [ ] 11-03-PLAN.md — dependency-free i18n catalog + context, functional language switcher, App shell + Settings dialog retrofit
...
| 11. Platform Polish | 2/2 | In Progress|  |
