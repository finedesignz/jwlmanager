...
- **D2-05 (CORRECTED 2026-07-21 per cross-AI review — over-deletion fix):** Delete removes **ONLY the selected `Note` rows** via `DELETE FROM Note WHERE NoteId IN (bound ids)` — it does NOT delete UserMark/BlockRange. Any resulting orphans (an UserMark no longer referenced by any Note *or* BlockRange, etc.) are swept by `trim_db` on save. Delete marks the session dirty; nothing is written to disk until save.
  `[auto] delete scope — Q: "Cascade everything in delete, or delete Notes + let trim_db sweep orphans on save?" → Selected: "Delete Notes only, trim sweeps orphans on save" (recommended default)`
  **Rationale:** Matches the Python app EXACTLY (`JWLManager.py:3666`: Notes delete is `DELETE FROM Note` only; FUNCTIONALITY-SPEC:140 confirms orphaned UserMarks are swept later by trim_db, not by the delete). The earlier wording ("delete the Note's own UserMark/BlockRange links") was an over-deletion vector: `Note.UserMarkId` is not unique and a UserMark can carry highlight BlockRange data the user did not select — deleting it in the delete op could destroy highlights. The dry-run preview accounts for both the direct Note deletes and the trim orphan cascade so the user sees the true effect.
