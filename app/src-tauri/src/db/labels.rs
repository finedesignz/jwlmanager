//! Shared resources.db label-synthesis helpers, reusable by every browse
//! category (Notes, Bookmarks, Annotations, Favorites, Highlights, Playlists).
//! Analog: the `process_code`/`process_color`/`process_detail` closures the
//! Python defines ONCE inside `regroup` (`JWLManager.py:578-639`) and all six
//! getters call — this module is their single Rust home (D6-01), so no
//! category re-implements the same math.
//!
//! These helpers are text-only: they build display labels from archive column
//! values and `ResourceCatalog` HashMap lookups. No SQL is constructed from
//! their output (T-06-01), and the archive-data path stays panic-free — the
//! only `expect` is on compile-time-constant regexes (T-06-02).
...
pub(crate) fn process_code(symbol: Option<&str>, issue: i64) -> (String, Option<String>) {
